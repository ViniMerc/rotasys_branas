...
export default class RideRepositoryDatabase implements RideRepository {
	...
	async hasActiveRideByAccountId(accountId: string): Promise<boolean> {
		const [rideData] = await this.connection.query("select count(*)::int as count from cccat17.ride where passenger_id = $1 and status in ('requested', 'accepted', 'in_progress')", [accountId]);
